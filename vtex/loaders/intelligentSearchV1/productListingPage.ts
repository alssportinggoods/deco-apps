...
type SearchV1Params = ISV1["GET /product-search/:facets"]["searchParams"];
...
export interface Props {
  ...
  /**
   * @title Show sponsored products
   * @description Include sponsored products in the results
   */
  showSponsored?: boolean;
  /**
   * @title Sponsored products count
   * @description Number of sponsored products to return
   */
  sponsoredCount?: number;
  /**
   * @title Advertisement placement
   * @description Where the sponsored products are displayed
   */
  advertisementPlacement?: SearchV1Params["advertisementPlacement"];
  /**
   * @title Repeat sponsored products
   * @description Allow sponsored products to also appear in the organic results
   */
  repeatSponsoredProducts?: boolean;
  /**
   * @title Remove hidden facets
...
/**
 * @title Product Listing Page - Intelligent Search v1
...
const loader = async (
  ...
  const span = ctx.monitoring?.tracer.startSpan(
    "intelligent-search-v1-metrics",
  );
  try {
    ...
    const params = withDefaultParams({ ...searchArgs, page, locale }, ctx, {
      "zip-code": searchArgs.zipCode,
      pickupPoint: searchArgs.pickupPoint,
    });
    ...
    const [productsResult, facetsResult] = await Promise.all([
      vcsDeprecated
        ["GET /api/intelligent-search/v1/product-search/*facets"]({
          ...params,
          showSponsored: props.showSponsored,
